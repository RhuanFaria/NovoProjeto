...
describe('API Loja Virtual - Produtos', () => {
  ...
  test('Deve retornar 404 e JSON no GET /produtos/id', async() => {
    const res = await req.get('/produtos/662a593c63cf39775ea61bba'); // 662a593c63cf39775ea61bbb
    expect(res.status).toBe(404);
    expect(res.type).toBe('application/json');
  });

  test('Deve retornar 200 e JSON no PUT /produtos/id', async() => {
    const res = await req.put(`/produtos/${id}`).send({nome: 'banana nanica', preco: 7.80});
    expect(res.status).toBe(200);
    expect(res.type).toBe('application/json');
  });

  test('Deve retornar 404 e JSON no PUT /produtos/id', async() => {
    const res = await req.put('/produtos/662a593c63cf39775ea61bba'); // 662a593c63cf39775ea61bbb
    expect(res.status).toBe(404);
    expect(res.type).toBe('application/json');
  });
